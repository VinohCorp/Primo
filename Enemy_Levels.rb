...
# ▼ Updates
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# 2014.07.31 - Added change level according to a variable (credits to VinohCorp)
...
class Game_Party < Game_Unit
  ...
  def match_party_level(level_type)
    ...
    when 4; return all_highest_level
  ...
  #--------------------------------------------------------------------------
  def variable
    #You can change the number of the variable, if you want
    lv = $game_variables[2]
    return lv
  end
